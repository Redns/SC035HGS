...
{
    #if IIC_PS_ENABLE
        return PS_IIC_Read_Reg(&camera->i2c_instance_ptr, camera->slv_addr, reg_addr);
    #else
...
{
    #if IIC_PS_ENABLE
        return PS_IIC_Write_Reg(&camera->i2c_instance_ptr, camera->slv_addr, reg_addr, value);
    #else
...
static s32 set_reg_bits(camera_t *camera, uint16_t reg_addr, uint8_t offset, uint8_t length, uint8_t value)
{
    ...
    // 获取寄存器原始值
    #if IIC_PS_ENABLE
        origin_value = PS_IIC_Read_Reg(&camera->i2c_instance_ptr, camera->slv_addr, reg_addr);
    #else
